...
import org.opennms.netmgt.provision.core.support.ClientConversation;
...
import org.opennms.poc.plugin.api.ServiceDetectorResultsImpl;
 ...
 * @version $Id: $
 */
public abstract class BasicDetector<Request, Response> extends AbstractDetector {
    
    private static final Logger LOG = LoggerFactory.getLogger(BasicDetector.class);

    @Getter
    private ClientConversation<Request, Response> conversation = new ClientConversation<Request, Response>();
    ...
    public CompletableFuture<ServiceDetectorResults> detect(ServiceDetectorRequest request) {
    	final String ipAddr = InetAddressUtils.str(request.getAddress());
        ...
        CompletableFuture<ServiceDetectorResults> future = new CompletableFuture<>();

        final Client<Request, Response> client = getClient();
        for (int attempts = 0; attempts <= retries; attempts++) {

            try {
                client.connect(request.getAddress(), port, timeout);
                ...
                if (attemptConversation(client)) {
                    return CompletableFuture.completedFuture(ServiceDetectorResultsImpl.builder().serviceDetected(true).build());
                }
        ...
        return CompletableFuture.completedFuture(ServiceDetectorResultsImpl.builder().serviceDetected(false).build());
    }
